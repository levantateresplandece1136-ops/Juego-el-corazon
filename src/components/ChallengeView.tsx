...
export const ChallengeView: React.FC<ChallengeViewProps> = ({
  ...
  const [isPreChallengePhase, setIsPreChallengePhase] = useState(true);
  ...
  // Check if this challenge involves secret words/mímica/dibuja/ahorcado
  const isSecretChallenge =
    challenge.category === 'mimica' ||
    challenge.category === 'dibuja' ||
    challenge.category === 'ahorcado' ||
    Boolean(challenge.targetText);
  ...
  const handleStartChallengeNow = () => {
    soundEngine.playSFX('fanfare');
    setIsPreChallengePhase(false);
    setIsTimerActive(true);

    if (presenterVoiceActive) {
      narratorEngine.speak(`¡El tiempo comienza ahora! ¡Mucha suerte ${player.name}!`);
    }
  };
  ...
  const getCategoryLabel = () => {
    switch (challenge.category) {
      case 'movimiento': return 'Movimiento y Ejercicio';
      case 'trae_objeto': return 'Búsqueda y Velocidad';
      case 'tres_cosas': return 'Rapidez Mental';
      case 'ahorcado': return 'Ahorcado en TV';
      case 'mimica': return 'Mímica e Interpretación';
      case 'dibuja': return 'Dibujo y Arte';
      case 'basta': return 'Basta y Palabras';
      case 'memoria': return 'Memoria y Concentración';
      case 'risa_prohibida': return 'Resistencia a la Risa';
      case 'no_digas_si': return 'Prohibido decir Sí o No';
      case 'cooperativo': return 'Trabajo en Equipo';
      default: return challenge.category.replace('_', ' ').toUpperCase();
    }
  };
  ...
  const preChallengeDialogue = `¡Atención ${player.name}! Tu siguiente reto es "${challenge.title}", de tipo ${getCategoryLabel()}. ${
    isSecretChallenge
      ? `¡Cuidado! Este reto contiene información secreta. Los demás jugadores no deben mirar la pantalla hasta que estés listo.`
      : `Tómense un momento para prepararse e inicien cuando estén listos.`
  }`;

  const activeChallengeDialogue = challenge.presenterPhrase || `¡Tiempo corriendo para ${player.name}! ¡Mucha suerte en ${challenge.title}!`;
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-4 sm:p-8 flex flex-col items-center justify-between select-none text-white relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto w-full z-10">
        <PresenterHost
          dialogue={isPreChallengePhase ? preChallengeDialogue : activeChallengeDialogue}
          mood={isPreChallengePhase ? 'excited' : 'celebrate'}
          autoSpeak={presenterVoiceActive}
        />
      </div>

      {/* PHASE 1: PRE-CHALLENGE ANNOUNCEMENT & PRIVACY SHIELD */}
      {isPreChallengePhase ? (
        <div className="max-w-3xl w-full bg-slate-900/95 border-4 border-yellow-400 p-6 sm:p-10 rounded-3xl shadow-2xl text-center flex flex-col items-center gap-6 z-10 my-4 animate-scale-up">
          
          <div className="flex flex-wrap items-center justify-center gap-2">
            <span className={`px-4 py-1.5 rounded-full font-black text-xs uppercase tracking-widest shadow ${getCategoryColor()}`}>
              ✦ TIPO DE RETO: {getCategoryLabel().toUpperCase()}
            </span>
          ...
          <div>
            <span className="text-xs font-black uppercase text-yellow-400 tracking-widest bg-yellow-400/20 px-3 py-1 rounded-full border border-yellow-400/40">
              JUGADOR EN TURNO: {player.name.toUpperCase()}
            </span>
            <h2 className="text-3xl sm:text-5xl font-black text-yellow-300 uppercase tracking-tight mt-3">
              {challenge.title}
            </h2>
          </div>

          {/* Description of What It's About */}
          <div className="bg-slate-950/90 p-5 rounded-2xl border-2 border-slate-700 max-w-xl w-full text-left">
            <h3 className="text-xs font-black text-yellow-400 uppercase tracking-wider mb-2">
          ...
          {/* SECRET WARNING BOX OR PUBLIC NOTICE */}
          {isSecretChallenge ? (
            <div className="bg-purple-950/90 border-4 border-purple-500 p-5 rounded-2xl max-w-xl w-full text-center flex flex-col items-center gap-2 shadow-xl animate-pulse">
              <span className="text-4xl">🙈 🔒</span>
              <h4 className="text-lg font-black text-purple-200 uppercase tracking-wide">
                ¡INFORMACIÓN SECRETA OCULTA!
              </h4>
              <p className="text-xs sm:text-sm font-bold text-purple-100 leading-normal">
                Este reto (como mímica o dibujo) contiene palabras secretas. Solo <strong className="text-yellow-300 underline">{player.name}</strong> debe estar frente a la pantalla. Los demás jugadores deben voltearse ahora.
              </p>
            </div>
          ) : (
            <div className="bg-emerald-950/80 border-2 border-emerald-500 p-4 rounded-2xl max-w-xl w-full text-center flex items-center justify-center gap-3 text-emerald-200 font-bold text-sm">
              <span className="text-2xl">📢</span>
              <span>¡Reto público familiar! Todos los participantes pueden ver y actuar juntos.</span>
            </div>
          )}

          {/* START BUTTON */}
          <button
            onClick={handleStartChallengeNow}
            className="w-full sm:w-auto px-10 py-5 bg-gradient-to-r from-yellow-400 via-amber-400 to-yellow-500 hover:from-yellow-300 hover:to-amber-300 text-slate-950 font-black text-xl sm:text-2xl uppercase rounded-2xl shadow-2xl border-4 border-white transform hover:scale-105 active:scale-95 transition cursor-pointer flex items-center justify-center gap-3 animate-bounce mt-2"
          >
            🚀 ¡LISTO, REVELAR SECRETO E INICIAR RETO!
          </button>
        </div>
      ) : (
        /* PHASE 2: FULL ACTIVE CHALLENGE RUNNER WITH TIMER & MINIGAMES */
        <div className="max-w-3xl w-full bg-slate-900/95 border-4 border-yellow-400 p-6 sm:p-8 rounded-3xl shadow-2xl text-center flex flex-col items-center gap-6 z-10 my-4">
          ...
          {challenge.targetText && (
            <div className="bg-yellow-400/20 border-2 border-yellow-400 p-3.5 rounded-2xl text-yellow-200 text-sm font-black shadow-lg animate-pulse">
              🎯 PALABRA / OBJETIVO SECRETO EN PANTALLA: <span className="text-white underline text-xl ml-1">{challenge.targetText}</span>
            </div>
          )}
